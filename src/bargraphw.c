...
PBL_APP_INFO(MY_UUID,
             "Bar Graph (white)", "Cameron MacFarland",
             1, 1, /* App version */
...
void update_hour(Layer *me, GContext* ctx) {
  ...
  graphics_context_set_fill_color(ctx, GColorBlack);/*changed from white > black*/
  graphics_context_set_stroke_color(ctx, GColorWhite);/*changed from black > white*/
...
void handle_init(AppContextRef ctx) {
  ...
  text_layer_init(&dateText, GRect(0,132,144,30));/*138*/
  text_layer_set_text_color(&dateText, GColorBlack); /*changed from white > black*/
  text_layer_set_background_color(&dateText, GColorClear);
  text_layer_set_text_alignment(&dateText, GTextAlignmentCenter);
  text_layer_set_font(&dateText, fonts_get_system_font(FONT_KEY_GOTHIC_24));
